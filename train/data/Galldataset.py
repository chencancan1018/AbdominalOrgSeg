...
class GallSegDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with torch.no_grad():
            ...
            # for gallbladder
            rand_z = random.randint(2, 7); rand_yx = random.randint(10, 60)
            pmin, pmax = self.find_valid_region(seg.copy(), values=[9], low_margin=[rand_z,rand_yx,rand_yx], up_margin=[rand_z,rand_yx,rand_yx])
            if pmin is None:
                pmin, pmax = self.find_valid_region(seg.copy(), values=[1, 9], low_margin=[rand_z,rand_yx,rand_yx], up_margin=[rand_z,rand_yx,rand_yx])
