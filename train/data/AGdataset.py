    ...
    def __getitem__(self, index):
        ...
        with torch.no_grad():
            ...
            seg = data["seg"]
             ...
             # for lag and rag location
            label = random.sample([7,8], 1)
            rand_z = random.randint(8, 15); rand_yx = random.randint(20, 60)
            if label == 7:
                pmin, pmax = self.find_valid_region(seg.copy(), values=[7], low_margin=[rand_z,rand_yx,rand_yx], up_margin=[rand_z,rand_yx,rand_yx])
                if pmin is None:
                    pmin, pmax = self.find_valid_region(seg.copy(), values=[1, 7], low_margin=[rand_z,rand_yx,rand_yx], up_margin=[rand_z,rand_yx,rand_yx])
                ...
                pmin, pmax = self.find_valid_region(seg.copy(), values=[8], low_margin=[rand_z,rand_yx,rand_yx], up_margin=[rand_z,rand_yx,rand_yx])
                if pmin is None:
                    pmin, pmax = self.find_valid_region(seg.copy(), values=[3, 8], low_margin=[rand_z,rand_yx,rand_yx], up_margin=[rand_z,rand_yx,rand_yx])
                vol = vol[pmin[0]:pmax[0], pmin[1]:pmax[1], pmin[2]:pmax[2]]
                seg = seg[pmin[0]:pmax[0], pmin[1]:pmax[1], pmin[2]:pmax[2]]
                seg_ = np.zeros(seg.shape, dtype=np.uint8)
                seg_[seg == 8] = 1
