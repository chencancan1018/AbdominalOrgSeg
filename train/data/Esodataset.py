...
class EsoSegDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with torch.no_grad():
            ...
            # for eso
            rand_z = random.randint(20, 30); rand_yx = random.randint(20, 40)
            pmin, pmax = self.find_valid_region(seg.copy(), values=[10], low_margin=[rand_z,rand_yx,rand_yx], up_margin=[rand_z,rand_yx,rand_yx])
            if pmin is None:
                pmin, pmax = self.find_valid_region(seg.copy(), values=[5, 10, 11], low_margin=[4,rand_yx,rand_yx], up_margin=[4,rand_yx,rand_yx])
